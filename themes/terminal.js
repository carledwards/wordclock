/* Terminal Theme with Monospace Grid
   Applies terminal styling with perfect character alignment and full justification.
...
(function() {
  ...
  // Character renderer for terminal theme
  function renderTerminalCharacter(li) {
  ...
  function restoreTerminalCharacter(li) {
  ...
  class TerminalTheme extends Theme {
    ...
    init() {
      // Apply monospace grid with terminal character renderer
      if (window.MonospaceGrid) {
        window.MonospaceGrid.applyMonospaceGrid('terminal', renderTerminalCharacter);
      } else {
        // Fallback: just render characters without grid
        const lis = document.querySelectorAll('.clock ul li');
        lis.forEach(li => renderTerminalCharacter(li));
      }
    ...
    setTitle(title) {
      // Terminal theme doesn't use title bar
    }
    ...
    cleanup() {
      ...
      if (window.MonospaceGrid) {
        ...
        // Fallback cleanup
        const lis = document.querySelectorAll('.clock ul li');
        lis.forEach(restoreTerminalCharacter);
      }
